...
const emptyForm = { date: '', coop_name: '', quantity: '', feed_kg: '', feed_cost: '', notes: '' }

export default function GalhasPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-green-900 mb-6">🐔 Galinhas / Ovos</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-green-100 p-5 mb-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Qtd. de Ovos *</label>
            <input type="number" required min="0" className="input-field" value={form.quantity}
              onChange={(e) => setForm({ ...form, quantity: e.target.value })} placeholder="0" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Galinheiro</label>
            <input type="text" className="input-field" value={form.coop_name}
              onChange={(e) => setForm({ ...form, coop_name: e.target.value })} placeholder="Ex: Galinheiro 1" />
          </div>
          <div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-green-50 text-green-800">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium">Galinheiro</th>
                <th className="px-4 py-3 text-right font-medium">Ovos</th>
              ...
              {records.length === 0 ? (
                ...
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-amber-50/50">
                    ...
                    <td className="px-4 py-3">{r.coop_name || '—'}</td>
